...
//register / add user
$save_to_db = mysqli_query($GLOBALS["___mysqli_ston"], "insert into `user`(`id`, `user_name`, `password`, `created_time`, `last_login`, `phone`, `site_funds_amount`, `role`, `picture`, `picture_thumbnail`, `secret_question`, `secret_answer`, `status`, `bank_account_no`, `bank_name`, `full_name`)
 VALUES ('', '{$username}', '{$hashed_password}', '', '', '{$phone}', '{$bonus_funds}','authenticated','','','{$secret_question}','{$secret_answer}','not verified','','','')") 
...
while($login_result = mysqli_fetch_array($login_query)){
	...
	$destination = BASE_PATH.'user/?user='.$_SESSION['username'].'&login=true';
