...
") or die('error installing notifications '.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false))); 


if($query) { status_message('success', "Successfully Activated!". $addon_name );}
block_create('General notifications','General Notifications','show_notifications("all");','notifications');
block_create('User notifications','User Notifications','show_notifications("all","week","user");','notifications');
...
$query = mysqli_query($GLOBALS["___mysqli_ston"], "DELETE fROM blocks WHERE parent_addon='system(notifications)'");
if($query){
	status_message('alert','All blocks removed!');
	}
	}
